...
class Net(nn.Module):
    def __init__(self):
        ...
        self.bottleneck2 = nn.Conv2d(in_channels=self.increase_rate * 8 * 9,  # 瓶颈层2,生成的特征矩阵作为亚像素卷积的输入
                                     out_channels=self.n_channels * self.scale ** 2, kernel_size=1,
                                     stride=1, padding=0, bias=False)
        self.bottleneck3 = nn.Conv2d(in_channels=self.n_channels,  # 瓶颈层3,相减的特征矩阵通过瓶颈层处理，结果作为亚像素卷积层的输入
                                     out_channels=self.n_channels * self.scale ** 2, kernel_size=1,
                                     stride=1, padding=0, bias=False)
    ...
    def forward(self, input):    # *input 会使输入不是 tensor
        ...
        lsr = self.bottleneck1(out)
        sub_out = torch.sub(input, lsr)
        out1 = self.bottleneck2(out)
        out2 = self.bottleneck3(sub_out)
